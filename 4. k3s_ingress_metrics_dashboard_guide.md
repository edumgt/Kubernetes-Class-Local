...
### 1-3) macOS hosts 파일에 도메인 매핑

macOS(관리자 권한)에서 파일 편집:
- `/etc/hosts`

맨 아래 추가:
```text
192.168.64.10  whoami.local
```
...
macOS 터미널:
```bash
ssh -L 8443:localhost:8443 ubuntu@192.168.64.10
```
...
브라우저:
- `https://localhost:9443` 또는 `https://192.168.64.10:9443`
