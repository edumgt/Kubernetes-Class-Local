...
> 목표: `https://192.168.64.10:8443/` 로 Dashboard가 안 열릴 때,
...
2) 8443이 **127.0.0.1(루프백)에만 바인딩**되어 외부(macOS)에서 접근 불가

특히 SSH 터널(`ssh -L 8443:localhost:8443 ...`)은 cp1 기준으로 **127.0.0.1:8443**만 잡는 경우가 많아,
그 상태에서 `https://192.168.64.10:8443`로는 절대 접속이 안 됩니다.
...
## 2) 외부(macOS)에서 보이게 port-forward 띄우기
...
정상 형태:
- `0.0.0.0:8443` 또는 `192.168.64.10:8443` 에서 `kubectl`이 LISTEN
...
macOS 터미널:
```bash
ssh -L 9443:localhost:9443 ubuntu@192.168.64.10
```
...
## 4) macOS에서 브라우저 전에 포트 체크

Terminal:
```bash
nc -vz 192.168.64.10 8443
```

- `succeeded` → 네트워크 OK, 이제 브라우저/인증서 경고 문제일 확률 높음
- 실패 → cp1 리스닝/바인딩/방화벽 문제

---

## 5) 브라우저 인증서 경고는 정상

`https://192.168.64.10:8443` 접속 시 “안전하지 않음/개인정보 보호 오류”가 뜰 수 있습니다(자체 서명).
