...
K3s 멀티노드(ubuntu 22.04, VirtualBox Host-Only 192.168.56.x) 
**Ingress(웹 라우팅) + Metrics Server(모니터링) + Kubernetes Dashboard(웹 UI)**까지 
...
### Kubernetes Dashboard(웹 UI) 설치 + 접속
...
cp1에서 직접 포트포워딩 / (B) Windows(호스트)에서 포트포워딩.
