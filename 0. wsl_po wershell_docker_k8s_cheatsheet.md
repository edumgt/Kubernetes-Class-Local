...
# WSL + PowerShell 네트워크 / Docker / Kubernetes 주요 명령어 치트시트 (예시 출력 포함)

> 목적: **WSL(리눅스)** + **PowerShell(윈도우)**에서 네트워크 점검/설정, 그리고 **Docker / Kubernetes(kubectl)** 운영 시 가장 자주 쓰는 명령어를 **실행 예시 + 출력 형태**로 빠르게 참고하기.

- 날짜: 2026-01-10
- 주의: 출력은 **환경(네트워크, 권한, OS, 클러스터 구성)**에 따라 달라집니다. 아래는 **형태를 익히는 샘플**입니다.
